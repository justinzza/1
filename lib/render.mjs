...
export function renderPage(registry, report) {
  ...
  const changed = changedKeys.map(key => byKey.get(key));
  ...
  const categories = Object.entries(CATEGORIES).map(([key, name], i) => {
    const events = changed.filter(event => event.category === key);
    const complete = report.category_coverage[key] === 'complete';
    const message = report.status === 'bootstrap' ? '尚未完成本期核验。' : complete ? '今日无重要增量。' : '今日无重要增量（仅指本次已核验内容；覆盖不完整，不代表所有来源均无新增）。';
    return `<section class="category-summary"><h3>${i + 1}）${e(name)}</h3>${events.length ? `<ul>${events.map(event => `<li><a href="#${e(event.id)}">${e(event.title)}</a><span class="summary-label">${e(evidenceLabel(event))}</span></li>`).join('')}</ul>${complete ? '' : '<p>仍有来源未完成检查，详见覆盖说明。</p>'}` : `<p>${message}</p>`}${report.category_notes?.[key] ? `<p class="category-note">${e(report.category_notes[key])}</p>` : ''}</section>`;
  }).join('\n');
...
<header class="topbar"><a class="brand" href="#top"><span class="brandmark">R</span><span>运营雷达</span></a><nav><a class="active" href="#daily">本期增量</a><a href="#actions">运营清单</a><a href="#news">事件档案</a></nav><div class="top-actions"><span class="status${report.status === 'bootstrap' ? ' pending' : ''}"><i></i>${report.status === 'bootstrap' ? '自动发布已接通 · 全源核验待运行' : report.status === 'partial' ? '部分来源未完成核验' : '最近一期已核验'}</span><div class="avatar">AM</div></div></header>
...
<section class="alert-card"><div class="alert-icon">!</div><div><p class="eyebrow">COVERAGE · 实际检查范围</p><h2>${title}</h2><p>${e(report.coverage_note)}</p></div><a href="#daily">查看分类摘要 →</a></section>
...
<section class="section top-changes"><p class="eyebrow">KEY CHANGES</p><h2>今日最重要的3项变化</h2>${top}</section><section class="section daily-summary" id="daily"><div class="section-head"><div><p class="eyebrow">DAILY BRIEFING</p><h2>分类摘要</h2></div></div>${categories}</section>
