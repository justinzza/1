...
export function renderPage(registry, report) {
  ...
  const categories = Object.entries(CATEGORIES).map(([key, name], i) => {
    ...
    const message = report.status === 'bootstrap' ? '尚未完成本期核验。' : complete ? '今日无重要增量。' : '本栏目核验不完整，不能据此判断无新增。';
    return `<section class="category-summary"><h3>${i + 1}）${e(name)}</h3>${events.length ? `<ul>${events.map(event => `<li><a href="#${e(event.id)}">${e(event.title)}</a><span class="summary-label">${e(evidenceLabel(event))}</span></li>`).join('')}</ul>${complete ? '' : '<p>仍有来源未完成检查，详见覆盖说明。</p>'}` : `<p>${message}</p>`}</section>`;
  }).join('\n');
