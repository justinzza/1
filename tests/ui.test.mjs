...
const displayEvents = registry.events.filter(event => event.marketplaces.some(market => displayMarkets.has(market)));
...
function fixture(storage = new Map(), blocked = false, hash = '') {
  const stories = displayEvents.map(ev => { const s = element({ eventKey: ev.event_key, marketplaces: ev.marketplaces.join(' '), dates: ev.history_dates.join(' '), priority: ev.priority, category: ev.category, search: `${ev.title} ${ev.summary}`.toLowerCase() }, ['story']); s.id = ev.id; s.textContent = `${ev.title} ${ev.summary}`; s.scrollIntoView = () => {}; return s; });
  const filters = ['all', 'P0', 'P1', 'P2'].map(priority => element({ filter: priority }));
  const views = ['latest', 'all'].map(view => element({ view }));
  const markets = ['all', 'US', 'CA', 'MX'].map(marketFilter => element({ marketFilter }));
  ...
  const nodes = { '#category-filter': category, '#date-filter': date, '#story-count': count, '#filter-empty': empty, '#reset-filters': reset, '#event-list': element({ currentEventKeys: report.new_event_keys.concat(report.updated_event_keys).join('\n') }), main: element({ reportDate: report.report_date || 'bootstrap' }) };
  const doc = { querySelector: sel => nodes[sel] || null, querySelectorAll: sel => ({ '#event-list .story': stories, '[data-filter]': filters, '[data-market-filter]': markets, '[data-view]': views, 'a[href^="#evt-"]': [], '.actions input[type="checkbox"]': [input] })[sel] || [], getElementById: id => stories.find(story => story.id === id) };
  ...
  return { stories, filters, views, markets, category, date, count, empty, reset, input, storage };
}
test('latest US changes are the default while archive filters remain complete', () => {
  ...
  assert.equal(f.count.textContent, String(displayEvents.filter(e => current.has(e.event_key)).length));
  f.markets.find(button => button.dataset.marketFilter === 'US').click();
  assert.equal(f.count.textContent, String(displayEvents.filter(e => current.has(e.event_key) && e.marketplaces.includes('US')).length));
  f.markets.find(button => button.dataset.marketFilter === 'all').click();
  assert.equal(f.count.textContent, String(displayEvents.filter(e => current.has(e.event_key)).length));
  f.category.change('ads');
  const adsCount = displayEvents.filter(e => current.has(e.event_key) && e.category === 'ads').length;
  assert.equal(f.count.textContent, String(adsCount)); assert.equal(f.empty.hidden, adsCount > 0);
  f.date.change('all');
  assert.equal(f.count.textContent, String(displayEvents.filter(e => e.category === 'ads').length));
  ...
  assert.equal(f.count.textContent, String(displayEvents.filter(e => current.has(e.event_key) && e.priority === 'P0').length));
  f.views.find(button => button.dataset.view === 'all').click();
  assert.equal(f.date.value, 'all');
  assert.equal(f.count.textContent, String(displayEvents.filter(e => e.priority === 'P0').length));
});
test('deep links reveal the single original event despite filters', () => {
  const current = new Set([...report.new_event_keys, ...report.updated_event_keys]);
  const foreign = displayEvents.find(e => !current.has(e.event_key));
...
test('daily briefing stays removed even when future reports contain category notes', () => {
  ...
  assert.equal((page.match(/data-event-key=/g) || []).length, displayEvents.length);
...
test('Canada and Mexico collection scope and filters preserve latest and archive views', () => {
  ...
  for (const [market, label] of [['CA', '加拿大站（CA）'], ['MX', '墨西哥站（MX）']]) {
    assert(html.includes(`data-market-filter="${market}"`));
    assert(html.includes(label.slice(0, 3)));
    const f = fixture();
    f.markets.find(button => button.dataset.marketFilter === market).click();
    const current = new Set([...report.new_event_keys, ...report.updated_event_keys]);
    const latestExpected = displayEvents.filter(event => current.has(event.event_key) && event.marketplaces.includes(market)).length;
    assert.equal(f.count.textContent, String(latestExpected));
    assert.equal(f.empty.hidden, latestExpected > 0);
    f.date.change('all');
    const archiveExpected = displayEvents.filter(event => event.marketplaces.includes(market)).length;
